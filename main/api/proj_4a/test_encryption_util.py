...
class TestEncryptionUtil(unittest.TestCase):
    ...
    def test_rsa_keypair(self):
        ...
        self.util.rsa_keypair = self.util.generate_key_pair(bytes)
    ...
    def test_rsa_encrypt_decrypt(self):
        ...
        logging.info("Original text: %s, encrypting using privkey %s", (plaintext, pubkey))
        encrypted = self.util.rsaEncryptToBase64(plaintext, pubkey)
        logging.info("Encrypted text: %s, decrypting using %s", (encrypted, privkey))
    ...
    def test_aes_decrypt_invalid_key(self):
        ...
        with self.assertRaises(ValueError):
            plaintext = self.util.aesDecryptFromBase64(encrypted, invalid_key)
